...
function preload(){
  ...
  play_img_1 = loadImage("play2.png");
...
function setup() {
  ...
  start_play.addImage("play2display",play_img_1);
...
function draw() {
  ...
  if(gameState === 0){
    ...
    if(mousePressedOver(manual)){
        ...
        start_play.changeImage("play2display",play_img_1);
        start_play.x = 1200;
        start_play.y = 870;
        if(mousePressedOver(start_play)){
          gameState = 1;
  ...
  // GameState is in play mode
  if(gameState === 1) {
  back_ground.changeAnimation("displayImage",back_image);
  back_ground.y = 100;
